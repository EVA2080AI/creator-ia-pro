...
const AntigravityBridgeNode = ({ id, data }: { id: string, data: AntigravityBridgeData }) => {
  ...
  return (
    <div className={`group relative rounded-[2.5rem] border border-white/5 bg-[#050506]/90 backdrop-blur-3xl w-[320px] animate-in slide-in-from-bottom-4 duration-500 nodrag shadow-3xl transition-all hover:border-[#d4ff00]/40 ${status === 'scanning' || status === 'cloning' ? 'ring-2 ring-[#d4ff00] shadow-[0_0_40px_rgba(212,255,0,0.3)]' : ''}`}>
      
      {/* Nebula V8.0 Antigravity Header */}
      <div className="flex items-center justify-between p-5 border-b border-white/5">
        <div className="flex items-center gap-3">
          <div className="p-2 rounded-xl bg-[#d4ff00] shadow-lg shadow-[#d4ff00]/20">
            <Rocket className="w-4 h-4 text-[#020203]" />
          </div>
          <div className="flex flex-col">
            <h3 className="text-[11px] font-black lowercase tracking-[0.1em] text-white">Antigravity Bridge</h3>
            <span className="text-[8px] font-black text-[#d4ff00] uppercase tracking-widest">Ecosystem_Cloner_V8</span>
          </div>
        </div>
      ...
      <div className="p-6 space-y-6">
        <div className="space-y-3">
          ...
          <div className="relative">
             <input
                ...
                className="w-full text-xs text-white bg-white/5 border border-white/5 p-4 pr-10 rounded-[1.5rem] focus:outline-none focus:ring-2 focus:ring-[#d4ff00]/20 transition-all font-bold"
             />
        ...
        <div className="grid grid-cols-2 gap-3">
           <button 
             ...
             className="flex items-center justify-center gap-2 px-5 py-4 rounded-2xl bg-[#d4ff00] hover:bg-[#c4eb00] text-[#020203] transition-all shadow-xl shadow-[#d4ff00]/20 disabled:opacity-50 active:scale-95 group/btn"
           >
           ...
           <button className="flex items-center justify-center gap-2 px-5 py-4 rounded-2xl bg-white/5 hover:bg-white/10 text-slate-300 border border-white/5 transition-all active:scale-95">
              <Braces className="w-3.5 h-3.5 text-[#d4ff00]" />
      ...
      <Handle type="source" position={Position.Right} className="!w-4 !h-4 !-right-2 !bg-[#d4ff00] !border-4 !border-[#050506] !shadow-2xl hover:scale-125 transition-transform" />
    </div>
  );
};
