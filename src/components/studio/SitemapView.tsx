      ...
      <div className="w-full max-w-3xl h-[80vh] bg-[#111114] rounded-2xl border border-white/[0.05] overflow-hidden flex flex-col shadow-2xl">
        ...
        <div className="px-6 py-4 border-b border-white/[0.05] flex justify-between items-center">
          <h3 className="text-white text-[14px] font-medium flex items-center gap-2">
            <Globe className="h-4 w-4 text-interactive" /> Sitemap del Proyecto
          </h3>
                ...
                {pages.map(page => (
                  ...
                  >
                    <div className="h-8 w-8 rounded-lg flex items-center justify-center shrink-0"
                      style={{ background: page.isHome ? 'rgba(52,211,153,0.12)' : 'rgba(var(--interactive-blue-rgb),0.1)', border: `1px solid ${page.isHome ? 'rgba(52,211,153,0.25)' : 'rgba(var(--interactive-blue-rgb),0.2)'}` }}>
                      {page.isHome ? <Home className="h-3.5 w-3.5 text-emerald-400" /> : <FileCode className="h-3.5 w-3.5 text-interactive" />}
                    </div>
