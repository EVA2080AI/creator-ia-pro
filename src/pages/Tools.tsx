  ...
  return (
    <div className="flex h-full w-full bg-background text-zinc-900 font-sans overflow-hidden">
      ...
      <aside className="hidden md:flex w-[260px] shrink-0 flex-col border-r border-zinc-100 bg-white/60 backdrop-blur-md shadow-[1px_0_20px_rgba(0,0,0,0.02)] overflow-hidden relative z-20">
        ...
        {/* Tool list */}
        <nav className="flex-1 overflow-y-auto px-4 pb-6 space-y-1 no-scrollbar">

          {/* Image section */}
          <div className="pt-4 pb-2">
            <span className="flex items-center gap-2 text-[9px] font-bold uppercase tracking-[0.25em] text-zinc-400">
              <Image className="h-3.5 w-3.5 opacity-70" /> Imagen & Visual
            </span>
          </div>
          {imageTools.map((tool) => {
            ...
            return (
              <button key={tool.id} onClick={() => switchTool(tool)} disabled={tool.disabled}
                ...
                <div className="flex-1 min-w-0 z-10">
                  <p className={cn(
                    "text-[12px] font-bold truncate leading-none font-display tracking-tight transition-colors",
                    isActive ? "text-primary" : "text-zinc-700"
                  )}>{tool.name}</p>
                  <p className="text-[10px] text-zinc-400 mt-1.5 uppercase tracking-widest font-medium">
          ...
          <div className="pt-6 pb-2">
            <span className="flex items-center gap-2 text-[9px] font-bold uppercase tracking-[0.25em] text-zinc-400">
              <FileText className="h-3.5 w-3.5 opacity-70" /> Copy & Texto
            </span>
          </div>
          {textTools.map((tool) => {
            const isActive = activeTool === tool.id;
