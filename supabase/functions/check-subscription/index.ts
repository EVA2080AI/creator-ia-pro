  ...
  try {
    ...
    try {
      if (subscription.current_period_end) {
        subscriptionEnd = new Date(Number(subscription.current_period_end) * 1000).toISOString();
      }
    } catch {
    ...
    await supabaseClient
      .from("profiles")
      .update({ subscription_tier: tierInfo.name })
      .eq("user_id", user.id);

    logStep("Success", { tier: tierInfo.name, credits: tierInfo.credits });
